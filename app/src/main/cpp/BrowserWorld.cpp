...
void
BrowserWorld::SetVRVideoPitch(const float aPitch) {
  if (m.vrVideo) {
    m.vrVideo->SetPitch(aPitch);
  }
}

float
BrowserWorld::GetVRVideoPitch() const {
  if (m.vrVideo) {
    return m.vrVideo->GetPitch();
  }
  return 0.0f;
}

bool
BrowserWorld::IsVRVideo180() const {
  if (m.vrVideo) {
    return m.vrVideo->Is180Projection();
  }
  return false;
}
...
JNI_METHOD(void, setVRVideoPitchNative)
(JNIEnv*, jobject, jfloat aPitch) {
  crow::BrowserWorld::Instance().SetVRVideoPitch(aPitch);
}

JNI_METHOD(jfloat, getVRVideoPitchNative)
(JNIEnv*, jobject) {
  return crow::BrowserWorld::Instance().GetVRVideoPitch();
}

JNI_METHOD(jboolean, isVRVideo180Native)
(JNIEnv*, jobject) {
  return crow::BrowserWorld::Instance().IsVRVideo180();
}
