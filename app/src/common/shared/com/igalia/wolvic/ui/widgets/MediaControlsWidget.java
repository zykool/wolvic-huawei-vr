...
import com.igalia.wolvic.ui.views.PitchControl;
...
public class MediaControlsWidget extends UIWidget implements WMediaSession.Delegate {
    ...
    private Handler mPitchCtrlHandler = new Handler(Looper.getMainLooper());
    private boolean mHidePitchSlider = false;
    private Runnable mPitchCtrlRunnable;
    ...
    private void updateUI() {
        ...
        mPitchCtrlRunnable = () -> {
            if ((mHidePitchSlider) && (mBinding.pitchControl.getVisibility() == View.VISIBLE)) {
                mBinding.pitchControl.setVisibility(View.INVISIBLE);
                stopPitchCtrlHandler();
            }
        };
        ...
        mBinding.mediaPitchButton.setOnClickListener(v -> {
            // Toggle pitch control visibility
            if (mBinding.pitchControl.getVisibility() == View.VISIBLE) {
                mBinding.pitchControl.setVisibility(View.INVISIBLE);
            } else {
                mBinding.pitchControl.setVisibility(View.VISIBLE);
                mHidePitchSlider = false;
                stopPitchCtrlHandler();
            }
            mBinding.mediaPitchButton.requestFocusFromTouch();
        });
        ...
        mBinding.pitchControl.setDelegate(new PitchControl.Delegate() {

            @Override
            public void onPitchChange(float aPitch) {
                if (mWidgetManager != null) {
                    mWidgetManager.setVRVideoPitch(aPitch);
                }
                mBinding.pitchControl.requestFocusFromTouch();
            }

            @Override
            public void onSeekBarActionCancelled() {
                mHidePitchSlider = true;
                startPitchCtrlHandler();
            }
        });
        ...
        mBinding.mediaPitchButton.setOnHoverListener((v, event) -> {
            float startY = v.getY();
            float maxY = startY + v.getHeight();
            if ((event.getX() <= 0) || (event.getX() >= v.getWidth()) || (!(event.getY() > startY && event.getY() < maxY))) {
                mHidePitchSlider = true;
        ...
        mBinding.pitchControl.setOnHoverListener((v, event) -> {
            float startY = 0;
            float maxY = startY + v.getHeight();
            if ((event.getX() > 0 && event.getX() < v.getWidth()) && (event.getY() > startY && event.getY() < maxY)) {
                mHidePitchSlider = false;
    ...
    public void setPitchButtonEnabled(boolean aEnabled) {
        mBinding.mediaPitchButton.setEnabled(aEnabled);
        if (!aEnabled) {
            mBinding.pitchControl.setVisibility(View.INVISIBLE);
        }
    }
    ...
    private void startPitchCtrlHandler() {
        mPitchCtrlHandler.postDelayed(mPitchCtrlRunnable, VOLUME_SLIDER_CHECK_DELAY);
    }

    public void stopPitchCtrlHandler() {
        mPitchCtrlHandler.removeCallbacks(mPitchCtrlRunnable);
    }
